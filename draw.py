import cv2 as cv
import numpy as np

blank = np.zeros((500,500,3), dtype='uint8')
cv.imshow('Blank', blank)

# 1. Paint the image a certain colour
#blank[200:300, 300:400] = 0,0, 255
#cv.imshow('Green', blank)
#img = cv.imread('Photos/cat.jpg')
#cv.imshow('Cat', img)

# 2. Draw a rectangle
#cv.rectangle(blank, (0,0), (250,500), (0,255,0), thickness=2 )
#cv.rectangle(blank, (0,0), (250,500), (0,255,0), thickness=cv.FILLED) #cv.rectangle(blank, (0,0), (250,500), (0,255,0), thickness=-1)
#cv.rectangle(blank, (0,0), (blank.shape[1]//2, blank.shape[1]//2), (0,255,0), thickness=cv.FILLED)
#cv.imshow('Rectangle', blank)
# 3. Draw a Circle

#cv.circle(blank, (blank.shape[1]//2, blank.shape[1]//2), 40,(0,0,255), thickness=3)
#cv.circle(blank, (blank.shape[1]//2, blank.shape[1]//2), 40,(0,0,255), thickness=-1)
#cv.imshow('Circle', blank)

# 3. Draw a Line

#cv.line(blank, (0,0), (blank.shape[1]//2, blank.shape[1]//2), (255,255,255), thickness=3)
#cv.line(blank, (100,250), (300,400), (255,255,255), thickness=3)
#cv.imshow('Line', blank)

# 4. Write Text on Image
cv.putText(blank,'Hello My name is Venu!!!', (0,225), cv.FONT_HERSHEY_TRIPLEX, 1.0, (0,255,0),2)
cv.imshow('Text', blank)
cv.waitKey(0)
